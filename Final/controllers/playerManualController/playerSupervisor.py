...
EPUCK_DIAMETER = .074
COLLISION_BUFFER = .02
# would be better to import them from the file, but then that causes robot to be initialized twice and that's just a mess
# from playerManualController import EPUCK_DIAMETER, COLLISION_BUFFER
...
def check_collisions():
    ...
    Similar code to moving_enemy_collision in playerController.py
    '''
    position = np.array(supervisor_get_robot_pose())
    enemies = np.array(supervisor_get_enemy_positions())

    distances = [np.linalg.norm(position[:2] - enemy_pos) for enemy_pos in enemies]
